 ...
 *)
	...
	struct
      ...
      let find_evals (var: symbol) (i: int) (nodes: node list): (symbol * value) Set.t =
        ...
        match i with
        | 0 -> if head_sym = var then head_evs else failwith "find_evals: i=0 refers to head but head symbol != var"
        | -1 ->
        ...
        | k when k > 0 ->
            ...
            if k > List.length matches then failwith "find_evals: var occurrence out of range among children"
            else let (_, evs) = List.nth matches (k - 1) in evs
        | _ -> failwith "find_evals: invalid index"
      ...
      let update_target (var: symbol) (i: int) (attr: symbol) (value: value) (nodes: node list): node list =
        ...
        | (head_sym, head_evs) :: children ->
            ...
            in
            match i with
            | 0 -> if head_sym <> var then failwith "update_target: i=0 targets head, but head symbol != var"
      ...
      let evaluateOp (op: string) (l: value) (r: value): value =
        ...
        | _ -> failwith (Printf.sprintf "Unknown operator or invalid operands: op=%S, left=%s, right=%s"
      ...
      let rec evaluate (head_sym: symbol) (e: expression) (nodes: node list): value =
        ...
        | Apply (attr, (var, i)) ->
            ...
            let evals =
              ...
              | Failure msg -> failwith (Printf.sprintf "evaluate: cannot resolve %s(%s[%d]) – %s"
      ...
      let eval (head_sym: symbol) (e: equation) (nodes: node list): node list =
        ...
        | Apply (attr, (var, i)), expr ->
            ...
            let value =
              ...
              | Failure msg -> failwith (Printf.sprintf "eval %s.%s[%d]: %s"
      ...
      let getRootRule (ag: t) (pt: parseTree): rule =
        match pt with
        | Leaf _ -> failwith "getRootRule"
        | Node (_, children) ->
            let head = getRootSymbol pt in
            let body = List.map getRootSymbol children in
            try Set.find (fun r -> r.head = head && r.body = body) ag.rules
      ...
      let apply_equations_at_head
        ...
        in
        (* If you want reproducible order, convert deterministically *)
        let eqs = SetUtil.to_list equations in
        List.fold_left
      ...
      let rec print_parse_tree pt =
        match pt with
        | Leaf (symbol, _) -> Printf.printf "Leaf: %s\n" (symb2str symbol)
     ...
     let eq_targets_child (head_sym : symbol) ((child_sym, k) : symbol * int) (eq : equation) : bool =
       ...
       | (Apply (_attr, (var, i)), _rhs) ->
           ...
           if i' = 0 then false
             ...
             let eff = if i = -1 then 1 else i in
             var = child_sym && eff = k
       | _ -> false
     ...
     let pick_child_node_by_occ ((sym, k) : symbol * int) (env_tail : node list) : node =
       let rec aux occ = function
         | [] ->
             failwith "calcAtributes: could not find targeted child in environment"
         | ((s, _) as n) :: tl ->
             if s = sym then
               let occ' = occ + 1 in
     ...
     let rec zip_update_children (children : parseTree list) (env_tail : node list) : parseTree list =
       ...
       | _ -> failwith "calcAtributes: arity mismatch between children and environment"
     ...
     let rec calcAtributes (ag : t) (pt : parseTree) : parseTree =
       ...
       | Node (((head_sym, _) as head), children) ->
           (* 0) Determine the matching rule by (head, symbols of children) *)
           let rule = getRootRule ag (Node (head, children)) in

           (* 1) Process children left->right, applying inherited equations per child before recursion *)
           let children' = eval_children_left_to_right ag head rule.equations children in

           (* 2) After children done, apply HEAD-targeted (synthesized) equations *)
           ...
           (* 3) Rebuild node from final environment *)
           match env3 with
     ...
     and eval_children_left_to_right
       ...
       let rec loop
         ...
         | child :: rest, occ :: occs_rest ->
             (* Build current environment: head :: processed_nodes ++ current_and_rest_nodes *)
             ...
             (* Inherited equations for THIS child only, using the current environment *)
             ...
             (* Extract updated node for THIS child from env_after_inh *)
             ...
             (* Recurse into child (which now has inherited attributes set) *)
